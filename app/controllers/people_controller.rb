class PeopleController < Devise::RegistrationsController
  ...
  def update
    if params[:custom_fields]
      old_custom_field_values = CustomFieldValueUtils.to_hash(values: @person.custom_field_values)
      FieldValueCreator.call(params[:custom_fields], @person)
      new_custom_field_values = CustomFieldValueUtils.to_hash(values: @person.custom_field_values.reload)
    end
  ...
  def destroy
    ...
    communities = @person.community_memberships.map(&:community_id)
    ...
    ActiveRecord::Base.transaction do
      ...
      communities.each { |community_id|
        PaypalService::API::Api.accounts.delete(community_id: @current_community.id, person_id: @person.id)
      }
    end
